...
Для доступа к API необходима аутентификация в домене .kontur.ru. Получив сессию по алгоритму, описанному ниже, необходимо во всех запросах к API передавать **в заголовках** куку (cookie) `auth.sid` со значением полученной сессии.
